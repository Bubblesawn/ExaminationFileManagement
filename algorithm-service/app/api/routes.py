...
@router.post("/material-preprocess", response_model=AlgorithmResponse)
def material_preprocess(request: MaterialPreprocessRequest) -> dict:
    """@brief 执行材料上传后的基础预处理。

    @param request 材料预处理请求。
    @return 材料格式校验、图片清晰度检测和基础分类统一响应。
    """
    return preprocess_material(request)
